...
parameterFileBSpline = ...
    "C:\Users\gimes\OneDrive\MAIA\3_UdG\classes\MIRA\lab\lab2\" + ...
    "elastix_model_zoo\models\Par0003\Par0003.bs-R7-ug.txt";
...
% -----------------------------------------------------------------
        ...
        if statusKeypoint == 0
            ...
            % Save the pts into a text file
            transformedCoordinates = extractOutputPoints(transformedResultPath);
            transformedCoordinatesPath = char( ...
                fullfile(caseOutputDirectory, 'transformedPts.txt'));
            writematrix(transformedCoordinates, transformedCoordinatesPath)
...
% ---------------------------------------------------------------------
    ...
    for fileIdx = 1:length(filesToDelete)
        if filesToDelete{fileIdx} ~= char(caseOutputDirectory)+"\."
